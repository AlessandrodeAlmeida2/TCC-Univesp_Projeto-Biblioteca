<template>
    <div class="aluno-dashboard">
      <header class="page-header">
        ...
        <div class="user-info">
          <img :src="alunoInfo.url_foto" alt="Foto do Aluno" class="profile-image" />
  ...
  export default {
    ...
    data() {
      return {
        ...
        emprestimos: [],
        reservas: [],
        alunoInfo: { id: null, nome: '', ra: '', url_foto: '' },
      };
    },
    async mounted() {
    ...
    methods: {
      async buscarAlunoInfo(ra) {
        ...
        const { data, error } = await supabase
          .from('alunos')
          .select('id, nome, ra, url_foto')
          .eq('ra', ra)
          .single();
        if (!error && data) {
          this.alunoInfo = { id: data.id, nome: data.nome, ra: data.ra, url_foto: data.url_foto };
        } else {
